...
\begin{center}
{\large\bfseries
SMART WASTE MANAGEMENET SYSTEM (ECOHAUL)
(EcoHaul)\par}
\end{center}
\vspace{2cm}
\noindent APPROVED BY::
...
\vspace{0.9cm}
\noindent Prof. Dr. Emin Erkan Korkmaz \dotfill

\vspace{0.9cm}
\noindent Asst. Prof. Mustafa Bülent Mutluoğlu \dotfill

\vfill
\noindent DATE OF APPROVAL:\quad /\quad /2025
\clearpage
...
\begin{center}
{\large\bfseries KENTSEL ÇEVRELER İÇİN AKILLI ATIK YÖNETİM SİSTEMİ (EcoHaul)}
\end{center}
\vspace{0.8cm}

\indent Bu proje, kentsel çevreler için yapay zeka destekli bir akıllı atık yönetim sistemi olan EcoHaul'un tasarımını ve uygulamasını sunmaktadır. Sistem, Dublin Belediyesi açık veri setinden elde edilen 3.424 gerçek atık konteyneri konumu üzerinde, yeni bir Dinamik Öncelik ve Verimli Kamyon Sevk (DPET) algoritmasını geleneksel sabit güzergâh yaklaşımıyla karşılaştırmak amacıyla paralel çift dünya simülasyon mimarisini kullanmaktadır. Bu mimaride iki bağımsız simülasyon dünyası özdeş çevresel koşullar altında eş zamanlı çalıştırılarak, gözlemlenen her performans farkının yalnızca sevkiyat stratejisine atfedilebilmesi sağlanır.

\indent DPET algoritması; trafik modellemesi, konteyner öncelik puanlaması, konteyner seçimi, kütle bilinçli güzergâh optimizasyonu, güzergâh verimlilik doğrulaması ve kamyon sevkiyatını kapsayan altı katmanlı bir işleme hattını entegre etmektedir. Tahmini Dolma Zamanı (TDZ) öğrenmesiyle desteklenen K-ortalamalar kümeleme tabanlı dolma tahmincisi, konteynerler dolup taşmadan önce proaktif sevkiyat kararları alınmasını mümkün kılar. Sistem ayrıca sabah ofis yoğunluğu, akşam alışveriş merkezi tepe saatleri ve konser, maç, festival gibi rastgele sivil etkinliklere dayalı, bölgeye özgü dolma hızı davranışlarını simüle eden bir kentsel çevre modeli içermektedir.

\indent React tabanlı etkileşimli bir pano, Leaflet harita altyapısıyla konteyner durumlarının, kamyon hareketlerinin, anomali olaylarının ve karşılaştırmalı anahtar performans göstergelerinin gerçek zamanlı görselini sunmaktadır. Bir IoT entegrasyon katmanı, fiziksel dağıtım için ESP32 mikrodenetleyicilerin JSN-SR04T su geçirmez ultrasonik sensörlerle birlikte simülasyonla aynı API üzerinden kullanılmasını desteklemektedir. 30 günlük simülasyonlardan elde edilen deneysel sonuçlar, DPET algoritmasının sabit güzergâh toplamaya kıyasla \%26,6 yakıt tasarrufu, \%75 daha az taşma olayı ve \%24,1 maliyet düşüşü sağladığını göstermektedir. Anomali tespit alt sistemi, altı kategori anomaliyi \%97,6 genel doğrulukla tespit etmiştir. Bu rapor; sistemin genel mimarisini, DPET algoritmasının matematiksel temellerini, simülasyon motorunun çalışma mantığını, IoT donanım katmanını ve deneysel değerlendirme sonuçlarını ayrıntılı olarak sunmaktadır.
...
\indent EcoHaul'un geliştirilmesine yönelik motivasyon, birbiriyle örtüşen üç önemli teknolojik ve toplumsal eğilimden kaynaklanmaktadır. Bu eğilimler, akıllı atık yönetimini hem teknik hem de ekonomik açıdan uygulanabilir kılmaktadır.
...
\indent \textbf {Part 2 (Related works)} The remainder of this report is organized as follows:
...
\indent \textbf{Part 3 (Analysis and Design)},In this part our architecture and design going to be introduced.We use a multilayer architecture with stack structure,simulation engine,sequence diagram use case model, and database entity-relationship schema explained with details.


\indent \textbf{Part 4 (Application)}, In this part we show the details and development decisions on ECO HAUL's multi layer structure.Dual world simulation engine, DPET Dispatch algorithm with its six layer structure,Median filtering and ETF based Fill predictor,Urban environment Model , anomaly detection sub system,ESP32 IoT hardware layer,FastAPI backend and React frontend layer explained with important details.


\indent \textbf {Part 5 (Test and Results)},In this part we show our test cases that we do on system and achieve some quantitative evaluation results ; Our main concerns in this part is comparison of DPET with alternative strategy  KPI analysis , dispatch route efficiency , anomaly detection rate and system performance metrics.All the concerns and their measurements discussed in this part of the report.

\indent  \textbf{Part 6 (Future Work)}
...
\section{Fill Prediction Module}
...
\node[box, fill=blue!15]   (iot)   at (0,0)     {IoT Katmanı\\(ESP32 Sensörler)};
\node[box, fill=teal!15]   (api)   at (5.5,0)   {REST API\\(FastAPI)};
\node[box, fill=green!15]  (ml)    at (5.5,-2.2){Dolma Tahmincisi\\(K-ort. + TDZ)};
\node[box, fill=orange!15] (dpet)  at (0,-2.2)  {DPET\\Algoritması};
...
\node[layer, fill=gray!20]    at (0,5.2)  {Katman 5: Ön Uç Pano (React 18 + Leaflet + Vite)};
\node[layer, fill=blue!12]    at (0,3.8)  {Katman 4: REST API (FastAPI + Uvicorn ASGI)};
\node[layer, fill=green!12]   at (0,2.4)  {Katman 3: Çift Dünya Simülasyon Motoru (asyncio)};
\node[layer, fill=orange!12]  at (0,1.0)  {Katman 2: Algoritmalar (DPET \textperiodcentered\ Tahminci \textperiodcentered\ Anomali)};
\node[layer, fill=red!10]     at (0,-0.4) {Katman 1: PostgreSQL 14 + PostGIS Veritabanı};
\end{tikzpicture}
...
\foreach \x/\lbl in {0/Ön Uç, 2.8/FastAPI, 5.6/Sim Motoru, 8.4/DPET, 11.2/PostgreSQL}{
  \node[life] at (\x, 0) {\lbl};
...
\draw[msg] (11.2,-2.3) -- node[above,font=\tiny]{bin durumları}   (5.6,-2.3);
...
node[right,font=\tiny]{dolma seviyesi güncelle} (5.6,-4.2);

\draw[msg] (5.6,-5.2) -- node[above,font=\tiny]{trigger\_dispatch(bins)} (8.4,-5.2);

\draw[msg] (8.4,-5.7) to[loop right]
node[right,font=\tiny]{puanla \& seç} (8.4,-5.7);

\draw[msg] (8.4,-6.4) to[loop right]
node[right,font=\tiny]{güzergâh kur (YK+2-opt)} (8.4,-6.4);

\draw[msg] (8.4,-7.1) -- node[above,font=\tiny]{sevkiyat planı}   (5.6,-7.1);

\draw[msg] (5.6,-7.8) -- node[above,font=\tiny]{INSERT olaylar}   (11.2,-7.8);

\draw[msg] (11.2,-8.3) -- node[above,font=\tiny]{commit OK}       (5.6,-8.3);

\end{tikzpicture}
\caption{EcoHaul simülasyon sevkiyat döngüsünün sıra diyagramı.}
\label{fig:sequence}
\end{figure}
...
\indent Bu sorgulama döngüsüne paralel olarak, simülasyon motorunun iç saati ilerler. Her tikta motor üç ana işi sırayla yürütür. İlk olarak \textbf{dolma işi} çalışır: her konteynerin doluluk seviyesi, mevcut simülasyon saatindeki kentsel çevre modeline göre güncellenir. Ardından \textbf{anomali işi} tüm konteynerleri anormal durum geçişleri, sensör arızaları ve taşma koşulları açısından kontrol eder.
...
\indent When the dispatch interval is triggered (every simulation hour in the DPET world), the engine initiates the DPET algorithm with the current container state list via the \texttt{trigger\_dispatch(bins)} call. DPET processes the containers through its six-layer pipeline: first scoring and selecting, then constructing the route using nearest neighbor and 2-opt. The algorithm returns a completed dispatch plan to the simulation engine. The engine creates simulated truck objects based on this plan; as the trucks follow the route, it records collection events and truck telemetry in the database using \texttt{INSERT} queries. When the database returns a commit OK, the loop completes and waits for the next tick.

\section{Use Case Diagram}

\indent Before we explain the functionalities in detail we create a use case diagram to show this functions and how they are interaction with differant actors in our system. Fig~\ref{fig:usecase} is the answer of  'what ECOHAUL do' for differant actors that have a part in this system
...
\indent Bu bölüm, sistemde saklanan verinin nasıl organize edildiğini, temel varlıkları ve bu varlıklar arasındaki ilişkileri göstermektedir. Veri modeli, veri bütünlüğünü sağlamak ve gereksiz veri tekrarını önlemek için normalizasyon ilkeleri gözetilerek tasarlanmıştır. EcoHaul veritabanı, PostGIS uzamsal uzantısıyla PostgreSQL üzerinde gerçeklenmiştir. Tablo~\ref{tab:db_tables}, beş birincil tabloyu ve amaçlarını açıklamaktadır.
...
\begin{lstlisting}[language=Python, caption=Basitleştirilmiş simülasyon tik döngüsü yapısı., label=lst:tick]
async def _tick_loop(self):
    while self._running:
        ...
        await self._run_fill_job()       # tum bin doluluk seviyelerini guncelle
        await self._run_anomaly_job()    # anomalileri tespit et
        await self._run_dispatch_job()   # DPET veya sabit guzergah tetikle
...
\indent İkinci katman, her aktif konteynere beş ağırlıklı bileşenden oluşan bileşik bir öncelik puanı $S$ atar:

\begin{equation}
  S = 0{,}40\,s_{\text{fullness}} + 0{,}35\,s_{\text{ETF}} + 0{,}12\,s_{\text{traffic}} + 0{,}08\,s_{\text{density}} + 0{,}05\,s_{\text{distance}}
  \label{eq:bin_score}
\end{equation}
\indent In this equation, each of the five components is normalized to the range $[0, 1]$ so that they can be combined on a common scale. The term $s_{\text{fullness}} = f/100$ is the  fullness level, where $f$ is the bin's current fill percentage. The term $s_{\text{ETF}} = \max(0,\,1 - \text{ETF}/12)$ expresses the urgency derived from prediction: as the Estimated Time to Full (ETF) approaches zero, this score approaches 1, whereas bins predicted to fill only after 12 hours or more contribute nothing, ensuring that imminent overflows dominate the ranking. The term $s_{\text{traffic}}$ is the traffic-adjusted availability score, which rewards bins that can be reached during low-congestion hours and penalizes those that would require travel through rush-hour traffic. The term $s_{\text{density}}$ captures the local fill concentration within the bin's K-means zone, favouring bins located in clusters where many neighbours are also filling up, so that a single trip can service a dense group of containers. Finally, $s_{\text{proximity}}$ rewards bins that are closer to the depot, reducing the dead-mileage a truck must travel before beginning useful collection.

\indent The five weights sum to 1.0, and the dominance of the fullness (40\%) and ETF (35\%) components reflects a deliberate design choice: together they account for 75\% of the total score, ensuring that the algorithm primarily prioritizes bins based on both their \textit{current} state and their \textit{predicted} future overflow risk. The remaining weights---traffic (12\%), density (8\%), and proximity (5\%)---act as secondary refinements that improve route efficiency without overriding the urgency signals. This weighting therefore balances the immediate need to empty full bins against the predictive goal of preventing overflows before they occur, while still optimizing the operational cost of each dispatch.
...
\indent The fifth layer checks whether a route is viable for shipment. A route is approved only if it satisfies the following condition:
\begin{equation}
  \eta = \frac{N_{\text{container}}}{D_{\text{total}}} \geq 0.20 \;\text{container/km}
  \label{eq:efficiency}
\end{equation}

\indent Routes that fall below this threshold are rejected; this prevents inefficient trips that collect only one or two containers over long distances. This filter serves as a critical checkpoint that prevents the system from wasting fuel unnecessarily.
...
\begin{equation}
  \hat{r} = \frac{\%100}{\bar{T}_{\text{loop}} / 3600}\;\; [\%/\text{saat}]
  \label{eq:fill_rate}
\end{equation}
...
\usepackage{booktabs} % Kalın ve ince çizgiler (toprule, midrule, bottomrule) için
\usepackage{float}    % [H] (kesin konumlandırma) parametresi için
...
\midrule
Simulation Durartion                            & 01/01/2024 -- 01/01/2025  \\
Run Per Algorithm (\(n\))        & 4 \\
Total bin Number                      & 3.424 (Dublin DCC veri kümesi) with 300 chosen bin \\
Label A (fast, 12--16 \%/sim-hour) & 20\% of the containers \\
Doluluk etiketi B (medium, 4--8 \%/sim-hour)    & 50\% of the containers\\
Doluluk etiketi C (slow, 0,5--1,5 \%/sim-hour) & 30\% of the containers \\
Max Fleet Size                 & 8/10/12/16 truck \\
Kamyon kapasitesi (normalize)                 & 1,0 (\%100 full container = \%5) \\
...
\begin{figure}[H]
    ...
    \includegraphics[width=\textwidth, height=0.95\textheight, keepaspectratio]{figures/23_kpi_tablo.png}
...
\indent Same data for easier reading given in the below with Table~\ref{tab:kpi_avg}
\begin{table}[H]
\centering
...
\begin{tabular}{lrrrr}
\toprule
\textbf{Metric (direction)} & \textbf{DPET} & \textbf{Greedy} & \textbf{2-opt} & \textbf{Dpet differances\,(G/2)} \\
\midrule
...
\indent When Table~\ref{tab:kpi_avg} and Figure~\ref{fig:overview} are evaluated together, DPET produces the best results on eight out of ten metrics. The remaining two metrics—overflow load and number of overflow events—work against DPET; this is detailed in Section~\ref{sec:tasma_trade_off} as a deliberate design choice.
...
\caption{System Efficiency Skor Dpet is the winner}
...
\indent Envorimental and econnomical effects shown in Şekil~\ref{fig:co2}, \ref{fig:yakit} , \ref{fig:maliyet}.In this three aspect Dpet algoritm is the most eco friendly solution for cities.
...
\caption{Filo büyüklüğüne göre sistem verimliliği --- her nokta bağımsız bir simülasyon koşusunu temsil eder.}
